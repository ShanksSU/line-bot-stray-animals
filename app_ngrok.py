...
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://admin:123456@127.0.0.1:5432/animals'
db = SQLAlchemy(app)
...
def handle_message(event):
    ...
    if len(list(query_data)) == 0:
        sql_cmd = "insert into users (uid, state, digit3, county) values('" + userid + "', 'no', 'no', 'no');"
    ...
    if user_text == '介紹':
        ...
        sql_cmd = "update users set state='hosp', county='no' where uid='" + userid +"'"
        db.engine.execute(sql_cmd)
        message = "進入查詢模式：動物醫院\n請輸入所在縣市"
        ...
        sql_cmd = "update users set state='shelter', county='no' where uid='" + userid +"'"
        db.engine.execute(sql_cmd)
        ...
        sql_cmd = "update users set state='adoption', county='no' where uid='" + userid +"'"
        db.engine.execute(sql_cmd)
        message = "進入查詢模式：動物領養\n請輸入所在縣市"
        line_bot_api.reply_message(
            event.reply_token,
        ...
        sql_cmd = "update users set state='no', county='no' where uid='" + userid +"'"
        db.engine.execute(sql_cmd)
        ...
        selected(event, user_text, userid, mode, conuty)
...
# 狀態選擇
def selected(event, user_text, userid, mode, conuty):
    if mode == 'hosp':
        if conuty != "no":
            print("鄉鎮市區")
            hosp_City(event, user_text, userid, conuty)
        else:     
...
def hosp_County(event, user_text, userid):
    try:
        ...
        if len(FlexMessage['contents']) > 10:
            sql_cmd = "update users set county='" + City + "' where uid='" + userid + "'"
            db.engine.execute(sql_cmd) 
...
# 醫院資訊輸出_鄉鎮市區
def hosp_City(event, user_text, userid, conuty):
    try:
        if user_text[0] == "台":
            user_text_new = user_text.replace("台", "臺")
        else:
        ...
        hosp_City_arr = []
        for i in range(len(Hosp_data)):
            if conuty == Hosp_data[i]["City"] and user_text_new in Hosp_data[i]["Address"]:
                FlexMessageNew['body']['contents'][0]['text'] = Hosp_data[i]["Name"]  #醫院名稱
                FlexMessageNew['body']['contents'][1]['text'] = Hosp_data[i]["Address"]      #醫院地址
